...
Partial Class iPxCrmUser_queryFollowDirect
    ...
    Public Function ExecuteQuery(ByVal cmd As SqlCommand, ByVal action As String) As DataTable
        ...
        End Using
    End Function
    Sub showdata_dropdownProductGrp()
        Dim cmd As New SqlCommand("select * from CFG_ProductGrp")
    ...
    Sub showdata_dropdownStatus()
        Dim cmd As New SqlCommand("select * from CFG_FollowupSts where StatusID > '1' and StatusID < '6'")
    ...
    Protected Sub btnLogin_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLogin.Click
        ...
        If txtQueryGuestName.Text.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (CFG_customer.CustName like '%" & Replace(txtQueryGuestName.Text.Trim, "'", "''") & "%') "
        End If

        If txtmemID.Text.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.TicketNo like '%" & Replace(txtmemID.Text.Trim, "'", "''") & "%') "
        End If

        If txtFrom.Text.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.SubmitFrom like '%" & Replace(txtFrom.Text.Trim, "'", "''") & "%') "
        End If
        ...
        If txtTransDate.Text.Trim <> "" Then
            ...
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.TDate between '" & transDate & " 00:00:00' and '" & transDate & " 23:59:00') "
        End If

        If dlProductGrp.SelectedValue.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.ProductGrp = '" & dlProductGrp.SelectedValue.Trim & "') "
        End If

        If dlProduct.SelectedValue.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.ProductID = '" & dlProduct.SelectedValue.Trim & "') "
        End If

        If dlMenu.SelectedValue.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.MenuID = '" & dlMenu.SelectedValue.Trim & "') "
        End If

        If dlSubMenu.SelectedValue.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.SubmenuID = '" & dlSubMenu.SelectedValue.Trim & "') "
        End If

        If dlStatus.SelectedValue.Trim <> "" Then
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.StatusID = '" & dlStatus.SelectedValue.Trim & "') "
        End If
        ...
        If txtPerUntl.Text.Trim <> "" Then
            ...
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.TDate <= '" & PerUntl & " 00:00:00') "
        End If
        ...
        If txtPerFrom.Text.Trim <> "" Then
            ...
            Session("sConditionDirect") = Session("sConditionDirect") & " AND (SPR_eticket.TDate >= '" & PerFrom & " 23:59:00') "
        End If

        If cbComment.Checked = True Then
            Session("sConditionDirect") = Session("sConditionDirect") & "and (SELECT c.isRead FROM SPR_eticketFollowUp as c where c.TicketNo=SPR_eticket.TicketNo and c.FollowUpDate in ( SELECT max(b.FollowUpDate) FROM SPR_eticketFollowUp as b where b.FollowUpCode ='H' group by b.TicketNo ))='1'"
        End If


        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "PopUser", "hideModal()", True)
        'sSQL = Session("sCondition")
        Response.Redirect("iPxCrmAssign.aspx?sConditionDirect=" & sSQL)
    End Sub

    Protected Sub btnExit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExit.Click
        Session("sConditionDirect") = ""
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "PopUser", "hideModal()", True)
        Response.Redirect("iPxCrmAssign.aspx?sConditionDirect=")
    End Sub
