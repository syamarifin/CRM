...
Partial Class iPxCrmMobile_iPxTicketCustomer
    ...
    Sub ListTicket()
        ...
        sSQL = "SELECT a.*,(select name from CFG_user where recID = a.AssignTo)as supportBy, ( SELECT isRead FROM SPR_eticketFollowUp where SPR_eticketFollowUp.TicketNo=a.TicketNo and FollowUpDate in ( SELECT "
        sSQL += "max(FollowUpDate) FROM SPR_eticketFollowUp where FollowUpCode ='P' group by TicketNo )) as coment from ( SELECT SPR_eticket.CustID, SPR_eticket.TicketNo, SPR_eticket.Tdate, SPR_eticket.SubmitFrom, SPR_eticket.Subject, SPR_eticket.CaseDescription, CFG_Product.ProductName, CFG_productMenu.MenuName, CFG_productSubMenu.SubmenuName"
        sSQL += ",CFG_FollowupSts.stsDescription, SPR_eticket.AssignTo From SPR_eticket "
    ...
    Sub deletedata()
        ...
        End If
        oSQLCmd = New SqlCommand(sSQL, oCnct)
        sSQL = "DELETE FROM SPR_eticket where TicketNo='" & Session("sCIdDelete") & "'"
        oSQLCmd.CommandText = sSQL
    ...
    Protected Sub gvTicket_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvTicket.RowCommand
        If e.CommandName = "getEditTiket" Then
            ...
            If oSQLReader.HasRows Then
                ...
                Session("sCIn8") = oSQLReader.Item("ProductGrp").ToString
                ...
                Session("sCIn7") = oSQLReader.Item("AttachFile").ToString
                Session("sCIn9") = oSQLReader.Item("Subject").ToString
                oCnct.Close()
                Response.Redirect("iPxCrmFormTicket.aspx")
            Else
                'Refreshh()
        ...
        ElseIf e.CommandName = "getDeleteid" Then
            ...
            'Session("sMessage") = "sure you want to delete data?| ||"
            'Session("sWarningID") = "0"
            'Session("sUrlOKONLY") = "iPxTicketCustomer.aspx"
            'Session("sUrlCANCEL") = "iPxTicketCustomer.aspx"
